...
from common.models.resnet import ResNet
...
#     def __init__(
#         self, backbone: ResNetFeatures, backbone_channels: int, representation_size: int
#     ) -> None:
#         super().__init__()
#         self.backbone = backbone

#         self.avg = torch.nn.AdaptiveAvgPool1d(1)
#         self.head = torch.nn.Linear(backbone_channels, representation_size)
#         # self.head = torch.nn.Sequential(
#         #     torch.nn.Linear(backbone_channels, 2 * representation_size),
#         #     torch.nn.ReLU(inplace=True),
#         #     torch.nn.BatchNorm1d(2 * representation_size),
#         #     torch.nn.Linear(2 * representation_size, representation_size),
#         # )

#     def forward(self, x: Tensor) -> Tensor:
#         x = x.permute(0, 2, 1)
#         x = self.backbone(x)[self.backbone.nodes[-1]]
#         x = self.avg(x)
#         x = x.squeeze(-1)
#         return self.head(x)
