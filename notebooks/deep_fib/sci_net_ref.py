...
class InteractorLevel(nn.Module):
    def __init__(
        ...
        if kernel % 2 == 0:
            pad_l = dilation * (kernel - 2) // 2 + 1  # by default: stride==1
            pad_r = dilation * (kernel) // 2 + 1  # by default: stride==1

        else:
            pad_l = (
                dilation * (kernel - 1) // 2 + 1
            )  # we fix the kernel size of the second layer as 3.
            pad_r = dilation * (kernel - 1) // 2 + 1
        ...
        self.P = InterMod(**kwargs)
        self.U = InterMod(**kwargs)
        self.phi = InterMod(**kwargs)
        self.psi = InterMod(**kwargs)
